...
class RingMoConfig(dict):
    ...
    Example:
        test.yaml:
            a:1
        >>> cfg = RingMoConfig('./test.yaml')
        >>> cfg.a
        1

        >>> cfg = RingMoConfig(**dict(a=1, b=dict(c=[0,1])))
        >>> cfg.b
        {'c': [0, 1]}
    """

    def __init__(self, *args, **kwargs):
        super(RingMoConfig, self).__init__()
        cfg_dict = {}
        ...
        for arg in args:
            if isinstance(arg, str):
                if arg.endswith('yaml') or arg.endswith('yml'):
                    raw_dict = RingMoConfig._file2dict(arg)
        ...
        RingMoConfig._dict2config(self, cfg_dict)
    ...
    def merge_from_dict(self, options):
        ...
        Examples:
            ...
            >>> cfg = RingMoConfig(**dict(model=dict(backbone=dict(type='ResNet'))))
        ...
        for full_key, value in options.items():
            ...
            for sub_key in key_list[:-1]:
                d.setdefault(sub_key, RingMoConfig())
                d = d[sub_key]
            sub_key = key_list[-1]
            d[sub_key] = value
        merge_dict = RingMoConfig._merge_a_into_b(option_cfg_dict, self)
        RingMoConfig._dict2config(self, merge_dict)
    ...
    def _merge_a_into_b(a, b):
        ...
        for k, v in a.items():
            if isinstance(v, dict) and k in b:
                b[k] = RingMoConfig._merge_a_into_b(v, b[k])
            else:
                b[k] = v
        return b
    ...
    def _file2dict(filename=None):
        ...
        if BASE_CONFIG in cfg_dict:
            ...
            for base_filename in base_filenames:
                cfg_dict_item = RingMoConfig._file2dict(
            ...
            # Merge config
            base_cfg_dict = RingMoConfig._merge_a_into_b(cfg_dict, base_cfg_dict)
        ...
        if isinstance(dic, dict):
            for key, value in dic.items():
                if isinstance(value, dict):
                    sub_config = RingMoConfig()
                    dict.__setitem__(config, key, sub_config)
                    RingMoConfig._dict2config(sub_config, value)
                else:
                    config[key] = dic[key]
