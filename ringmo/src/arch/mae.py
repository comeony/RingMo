...
class Mae(nn.Cell):
    ...
    def __init__(self, encoder, decoder_layers=12, decoder_num_heads=16, decoder_dim=512, norm_pixel_loss=False):
        ...
        self.patchify = Patchify(patch_size=encoder.patch_size)
        self.unpatchify = UnPatchify(
            patch_size=encoder.patch_size, seq_length=encoder.num_patches)

        self.add = P.Add()
        self.cat = P.Concat(axis=1)
        self.stride_slice = P.StridedSlice()
        self.stride_slice4d = P.StridedSlice()
        ...
        self.mse_loss = MSELoss(norm_pixel_loss)
        self.add2 = P.Add().shard(((), ()))

        self.images_summary = P.ImageSummary()
